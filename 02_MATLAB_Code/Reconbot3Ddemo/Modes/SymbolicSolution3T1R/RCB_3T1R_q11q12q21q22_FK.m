...
q11q12q21q22 = [pi/6, pi/3, pi/3, pi/3];
...
% For real elements of X, atan(X) returns values in the interval [-pi/2,pi/2]
 q13all = 2 * atan(x) - q12;
 for k = 1:length(x)
         q13all(k*2 + 1) =  2 * (- pi + atan(x(k))) - q12;
         q13all(k*2 + 2) =  2 * (  pi + atan(x(k))) - q12;
 end
%  q13all*180/pi;
%  q13all = (q13all*180/pi + 180)*pi/180
%  q12*180/pi
...
%      q23all * 180 / pi
    ...
    if asin(sin(q12) + sin(q12 + q13SingleValue) - sin(q22)) < 0
        q23all(2) =  - pi - asin(sin(q12) + sin(q12 + q13SingleValue) - sin(q22)) - q22;
    elseif asin(sin(q12) + sin(q12 + q13SingleValue) - sin(q22)) > 0
        q23all(2) =  pi - asin(sin(q12) + sin(q12 + q13SingleValue) - sin(q22)) - q22;
    else
    end
    q23all * 180 / pi;
