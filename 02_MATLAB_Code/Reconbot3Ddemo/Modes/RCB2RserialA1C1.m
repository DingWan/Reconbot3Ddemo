classdef RCB2RserialA1C1
    ...
    methods
        ...
        function [p, EulerAngle_q11_theta, ABC_FeasibleSolution, q1q2_FeasibleSolution, WSvalue] = RCB_2R_SerialA1C1_IK(obj)
            % p = [x, y, [], [], [], []]; y < 0
            L1 = obj.l1;
            L2 = obj.l2;
            po = obj.pos;
...
%                 angleA1B1C1 = 0;
        ...
        function [p, ABC, q1q2] = RCB_2R_serialA1C1_FK(obj)
            ...
            for i = 1:2
                ...
                if q11 >= pi/2 && q11 <= 3*pi/2
                    if i == 1
                        q21 = q11 - pi;
                    else
                        q21 = q11 + pi;
                    end
                elseif q11 >= -3*pi/2 && q11 <= -pi/2
                    if i == 1
                        q21 = q11 + pi;
                    else
                        q21 = q11 - pi;
              ...
              %% ----------------------Calculate the RPY angle--------------------------
                theta = pi/2 - q24 - (q22 + q23); % q24 = pi/2 - (q22 + q23 + theta);
                u_RotationAxis = [cos(q11), sin(q11), 0];
                r = [u_RotationAxis, theta];
                %-- m=vrrotvec2mat(r):Convert rotation from axis-angle to matrix representation--
                Matrix_from_axis_angle = vrrotvec2mat(r);
