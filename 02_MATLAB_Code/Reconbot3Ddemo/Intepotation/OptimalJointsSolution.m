...
for jj = 1:length(q1q2(:,1))
    % Here, for the first step, modes 1-9 execute once (length(MPOTP_cell) = 2); Modes 10/11
    % execute twice (length(MPOTP_cell) = 3), which is judged by i < length(MPOTP_cell)
    if  j == 1 && (Self_adjustment_Enable_Disable == 1 || Self_adjustment_Enable_Disable == 2) && i < length(MPOTP_cell)
    ...
    elseif j == 2 && Mode_current ~= 5 && (Self_adjustment_Enable_Disable == 1 || Self_adjustment_Enable_Disable == 2) && i < length(MPOTP_cell)
