...
for IntepPointNum = 1 : length(MPOTP_cell)
    ...
    % The way is,
    % 1. intepolate the current to next step, and get the required q11 and q21 value (it will be the value of the second intepolation step) 
    % 2. Assign the obtained value q11 and q21 to self-adjustment motion planning with intepotation between start value to obtained value;
    % 3. Execute the self-adjustment first, and then, carry out the required motion 
    
    if MPOTP_cell{IntepPointNum}{1} == 5 && IntepPointNum + 1 <= length(MPOTP_cell) ...
        && MPOTP_cell{IntepPointNum + 1}{1} ~= 5%(MPOTP_cell{IntepPointNum + 1}{1} <   3 || MPOTP_cell{IntepPointNum + 1}{1} > 5)
        % sum(cellfun('isempty',MPOTP_cell{IntepPointNum+1}) == 0) is the total number of non-empty array
        
    elseif IntepPointNum - 1 >= 1 && MPOTP_cell{IntepPointNum - 1}{1} == 5 ...
         && MPOTP_cell{IntepPointNum}{1} ~= 5%(MPOTP_cell{IntepPointNum}{1} < 3 || MPOTP_cell{IntepPointNum}{1} > 5)
        % First assign the second intepotation value to the first row, to make it the same
...
T = IntepPointNum;
MPOTP_cell_1 = MPOTP_cell;
q0q1q2_mat_1 = q0q1q2_mat;
%% Last step for returnning to HomePosition
IntepPointNum = T;
MPOTP_cell = MPOTP_cell_1;
q0q1q2_mat = q0q1q2_mat_1;
