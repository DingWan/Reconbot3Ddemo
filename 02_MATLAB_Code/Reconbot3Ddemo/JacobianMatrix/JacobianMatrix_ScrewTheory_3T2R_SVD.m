...
%%
po_start = { 0.100 0.100 0.23 0*pi/180 [] []};
po_end = { -0.10 0.100 0.15 0*pi/180 [] []};
Time = [0 5];
n = 20;
...
%     err = cell2mat(POend) - cell2mat(POstart);    
    ...
    %while norm(err)>0.001        
       %% ============================ IK ==============================
        ...
        det_Jc_Ob_3T2R(i) = det(J_Ob_3T2R(5:6,1:2)/norm(J_Ob_3T2R(5:6,1:2))); % normized
...
%%
figure(3)
[~,col_q1] = find(abs(det_Jq1_Ob_3T2R) == max(abs(det_Jq1_Ob_3T2R)));
[~,col_Ob] = find(abs(det_J_Ob_3T2R) == max(abs(det_J_Ob_3T2R)));
det_Jq1_Ob_3T2R = det_Jq1_Ob_3T2R/abs(det_Jq1_Ob_3T2R(col_q1));
det_J_Ob_3T2R = det_J_Ob_3T2R/abs(det_J_Ob_3T2R(col_Ob));
i = 1:n;
plot(i,det_Jq1_Ob_3T2R,'r-');hold on % with redundant actuation: det_Jq1_Ob_3T1R<0.2
plot(i,det_Jc_Ob_3T2R,'g-');hold on
plot(i,det_J_Ob_3T2R,'b-');hold on
grid on
