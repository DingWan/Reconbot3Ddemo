...
%% Calculate the IK Jacobian
    ...
    J_dx_dq_eul(:,i) = [  dq(1,i) / deltapo; %This whole thing is a single column
                          dq(2,i) / deltapo;
                          dq(6,i) / deltapo;
                          dq(7,i) / deltapo;
                          0;
...
s_Jc_Ob15 = svd(J_Ob_3T1R(5:6,:));
s_J = svd(J_Ob_3T1R);

determint = det(J_Ob_3T1R)
