...
% validate inputs
validateattributes(x, {'double'}, {'2d','nonempty','finite','nonnan'},...
    'delayseq', 'X');
validateattributes(dtIn, {'double'},...
    {'vector', 'real', 'finite', 'nonnan'}, 'delayseq', 'DELAY');

if nargin > 2
    eml_assert_no_varsize(1:nargin, x,dtIn,fs);
    ...
    eml_assert_no_varsize(1:nargin, x,dtIn);
end
...
if isreal(x)
   ...
   output = complex(zeros(maxLengthLimit,length(dt)));
end
