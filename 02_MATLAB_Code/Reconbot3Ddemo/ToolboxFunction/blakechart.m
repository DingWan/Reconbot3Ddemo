...
par.addParamValue('ScalePower',defaultScalePower);
par.addParamValue('SurfaceRefractivity', defaultRefractivity);
par.addParamValue('RefractionExponent', defaultExponent);
...
ht = 10^exps;   % Starting point
for k = exps:expe-1
    ht = [ht linspace(2*10^k, 10^(k+1), 9)]; 
end
htx = ht(end) + 10^(k+1);
...
rng = 10^exps;   % Starting point
for k = exps:(expe-1)
    rng = [rng linspace(2*10^k, 10^(k+1), 9)];
end
