function sv = steervec(pos_in,ang_in)
%steervec Steering vector for a sensor array 
%   SV = steeringvec(POS,ANG) returns the steering vector of a sensor
%   array defined in POS, for the directions specified in ANG (in degrees).
%   SV is an NxM matrix, where N is the number of elements in the sensor
%   array and M is the number of incoming signals. Each column of SV
%   contains the steering vector of the array for the corresponding
%   direction specified in ANGLE.
%
...
[pos,ang] = parseInput(pos_in,ang_in);

sv = phased.internal.steeringvec(pos,1,1,ang);


%---------------------------------
function [pos,ang] = parseInput(pos_in,ang_in)
eml_assert_no_varsize(1,pos_in,ang_in);
if size(pos_in,1) == 1
