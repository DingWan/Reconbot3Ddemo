function w = mvdrweights(pos_in,ang_in,cov)
...
[pos,ang,N_elem,N_angl] = parseInput(pos_in,ang_in,cov);

Constraints =  phased.internal.steeringvec(pos,1,1,ang);

w = complex(zeros(N_elem,N_angl));
...
for m = N_angl:-1:1
    AS = Constraints(:,m);
    covinvAS = cov\AS;
    w(:,m) = covinvAS/real(AS'*covinvAS);     %MVDR weights
end

%---------------------------------
function [pos,ang,N_elem,N_angl] = parseInput(pos_in,ang_in,scov_in)
eml_assert_no_varsize(1:nargin, pos_in,ang_in,scov_in);
if size(pos_in,1) == 1
