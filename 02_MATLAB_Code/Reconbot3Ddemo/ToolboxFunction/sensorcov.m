function xcov = sensorcov(pos_in,ang_in,ncov_in,scov_in)
%sensorcov Covariance matrix of received signal
%   XCOV = sensorcov(POS, ANG) returns the received covariance matrix,
%   XCOV, of narrowband plane wave signals at a sensor array. POS
%   represents the element positions of the sensor array. ANG represents
%   the incoming directions of each plane wave signal.
%
...
%
%   XCOV is an NxN matrix covariance matrix of the received signal at the
%   sensor array.
%
%   XCOV = sensorcov(POS,ANG,NCOV) specifies the noise characteristics in
...
%   value of NCOV is 0.
%
%   XCOV = sensorcov(POS,ANG,NCOV,SCOV) also specifies the incoming signal
...
%   elementPos = (0:N-1)*d;
%
%   xcov = sensorcov(elementPos,[30 60],db2pow(-10));
%   doa = rootmusicdoa(xcov,2)
%
...
xcov = sv*scov*sv' + ncov;
xcov = (xcov+xcov')/2;  % ensure Hermitian
