...
defaultunitStr = 'm';    % meters
 ...
 if isempty(coder.target)
     p = inputParser;
     p.addParamValue('RCS',defaultRCS);   
     p.addParamValue('Ts', defaultTs); 
     p.addParamValue('Gain',defaultGain); 
     p.addParamValue('Loss',defaultLoss);  
     p.addParamValue('unitstr',defaultunitStr);       
     p.parse(varargin{:});
     RCS = p.Results.RCS;
     Ts = p.Results.Ts;
     Gain = p.Results.Gain;
